...
class MDTestStats(BaseModel):
    create_rate: Optional[float] = None
    create_time: Optional[float] = None
    stat_rate: Optional[float] = None
    stat_time: Optional[float] = None
    read_rate: Optional[float] = None
    read_time: Optional[float] = None
    rename_rate: Optional[float] = None
    rename_time: Optional[float] = None
    remove_rate: Optional[float] = None
    remove_time: Optional[float] = None


class MDTestResultDetail(BaseModel):
    rank: int  # task rank, -1 = summary of all
    items: int
    dir_stats: MDTestStats
    file_stats: MDTestStats
    tree_stats: MDTestStats


class MDTestCoreOutput(BaseModel):
    """
    MDTestCoreOutput represents the output processed directly from
    mdtest's CSV file.
    """

    results: List[MDTestResultDetail]


class MDTestOutput(MDTestCoreOutput):
    """
    MDTestOutput represents the mdtest output along with useful
    metadata.
    """

    # results: List[MDTestResultDetail]  # inherited
    nodes: int
    tasks_per_node: int
    total_tasks: int
    command_line: str
...
def parse_mdtest_result(
    ...
    """
    Parse an MDTest output CSV string into an MDTestOutput object.

    Unlike IOR, mdtest does not provide benchmark metadata such as the
    node count and command line in its output, so these must be tracked
